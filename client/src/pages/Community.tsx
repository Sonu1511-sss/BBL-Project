...
const placeholderThreads: Thread[] = [
  {
    _id: 'demo-1',
    title: 'Best approach to sliding window problems?',
    content: 'Struggling with variable window sizing. Any tips on pattern recognition?',
    author: { name: 'Aditi' },
    courseId: { title: 'DSA Patterns', slug: 'dsa-patterns' },
    upvotes: [{}, {}, {}],
    replies: [{}, {}],
  ...
  },
  {
    ...
    content: 'Token bucket vs leaky bucket — which to pick for bursty traffic?',
    author: { name: 'Rahul' },
    courseId: { title: 'System Design', slug: 'system-design' },
    upvotes: [{}, {}, {}, {}],
    replies: [{}],
    createdAt: new Date().toISOString(),
  },
  {
    _id: 'demo-3',
    title: 'DBMS indexing question',
    content: 'How to decide between B+ tree and hash index for mixed workloads?',
    author: { name: 'Neha' },
    courseId: { title: 'DBMS', slug: 'dbms' },
    upvotes: [{}, {}],
    replies: [{}, {}, {}],
    createdAt: new Date().toISOString(),
  },
  {
    ...
    content: 'Is Bankers algorithm practical in real systems? Alternatives?',
    author: { name: 'Karan' },
    courseId: { title: 'OS', slug: 'os' },
    upvotes: [{}],
    replies: [],
    createdAt: new Date().toISOString(),
  },
...
export default function Community() {
  ...
  const fetchThreads = async () => {
    try {
      ...
      setThreads(data.length ? data : placeholderThreads);
    } catch (error) {
      console.error('Failed to fetch threads:', error);
      toast.error('Failed to load discussions. Showing sample threads.');
    ...
    <div className="bg-dark-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-3">
          {threads.map((thread) => (
            <div key={thread._id} className="card p-5 hover:border-[#4285F4]/60 transition-colors">
